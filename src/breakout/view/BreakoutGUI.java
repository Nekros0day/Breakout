...
public class BreakoutGUI extends Application implements IEventHandler {
        ...
        switch (kc) {
            case LEFT:
                if(paddle.canMoveToPositionLeft()){
                    paddle.updatePaddleXPosition(-Paddle.PADDLE_SPEED);
                }
                break;
            case RIGHT:
    ...
    private void keyReleased(KeyEvent event) {
        ...
        switch (kc) {
            case LEFT:
                if(paddle.canMoveToPositionLeft()){
                    paddle.paddleDeteriate(-Paddle.PADDLE_SPEED);
                }
                break;
            case RIGHT:
                if(paddle.canMoveToRight()){
    ...
    private void newGame() {
        ...
        breakout = new Breakout(new Ball(), getWalls(), new Paddle(), bricks);
    ...
    private List<Wall> getWalls() {
        Wall left = initializeWallPositions(Wall.Dir.VERTICAL, 0);
        Wall top = initializeWallPositions(Wall.Dir.HORIZONTAL, 0);
        Wall right = initializeWallPositions(Wall.Dir.VERTICAL, 400);
        return Arrays.asList(left, top, right);
    }

    private Wall initializeWallPositions(Wall.Dir direction, double xPos){
        double wallWidth = 3;
        double width = 0;
        double height = 0;
        double x = 0;
        ...
        double y = 0;
        double y1 = 0;
        if(direction == Wall.Dir.HORIZONTAL){
            width = Breakout.GAME_WIDTH;
            height = wallWidth;
            x = 0;
            x1 = x + Breakout.GAME_WIDTH;
            y1 = y + height;
        }
        else{
            ...
            if(xPos >= Breakout.GAME_WIDTH / 2 ){
                x = Breakout.GAME_WIDTH - wallWidth;
                x1 = x + wallWidth;
            }
            else{
                x = 0;
        ...
        Wall wall = new Wall(direction, x, y, x1, y1, width, height);
        return wall;
    }
